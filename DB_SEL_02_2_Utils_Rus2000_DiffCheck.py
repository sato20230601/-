...
def get_unique_dates(cursor,logger):
    ...
    logger.info(f"rus2000から取得年月日の取得処理を開始します。")
    ...
    logger.debug(f"rus2000から取得年月日取得クエリ: {query}")
    ...
    logger.info(f"rus2000から取得年月日の取得処理を終了しました。")
    return unique_dates
...
def get_diff_records(cursor, old_date, new_date,logger):
    ...
    try:
        ...
        ON
          NEW_rus2000.Symbol = OLD_rus2000.Symbol
        WHERE
          OLD_rus2000.Symbol IS NULL
        """
...
# 差分チェック結果を「rus2000_diff_record」に登録を行う。
def insert_diff_records(cursor, diff_records, logger):
    ...
    try:
        ...
        for record in diff_records:
            insert_query = """
            INSERT INTO `rus2000_diff_record` (`Date_YYYYMMDD`, `Symbol`, `Action`, `INS_DATE`)
                VALUES (%s, %s, %s, %s)
...
def Rus2000_DiffCheck_process_data(logger):

    has_error = False  # エラーフラグを初期化
    try:
        logger.info(f"rus2000の取得年月毎の差分チェックを開始します。")
        ...
        # 差分チェック処理結果を「rus2000_diff_record」に登録
        ...
        cnx.close()
        logger.info(f"rus2000の取得年月毎の差分チェック処理が完了しました。")
